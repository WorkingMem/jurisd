...
/** Exa is a paid API (generous quotas); a light cap just avoids accidental bursts. */
 ...
 */
export function canonicaliseAustliiUrl(raw) {
    let parsed;
    try {
        parsed = new URL(raw);
    }
...
function neutralCitationFromText(text) {
 ...
 */
export async function searchAustliiViaExa(query, options, limit) {
    ...
    try {
        ...
        // Network/timeout/parse failures degrade to "no Exa results".
        return [];
    }
    ...
    for (const r of json.results ?? []) {
        ...
        if (wantLegis ? !isLegis : !isCase)
            continue;
        if (seen.has(url))
            continue;
        seen.add(url);
        const title = (r.title ?? "").trim() || url;
        const neutralCitation = neutralCitationFromText(title) ?? austliiUrlToNeutralCitation(url) ?? undefined;
        out.push({
            title,
            citation: undefined,
            neutralCitation,
