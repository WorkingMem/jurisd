...
import { isAustliiUrl, toClassicDocUrl, austliiUrlToNeutralCitation, austliiUrlIsLegislation, } from "./austlii-url.js";
...
export async function fetchDocumentText(url) {
    ...
    if (isSourceUrl(url)) {
        if (!config.source.sessionCookie) {
            throw new Error("fetch_document_text requires SESSION_COOKIE for removed.invalid URLs. " +
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (status === 401 || status === 403) {
                    throw new Error(`removed.invalid returned ${status}. The SESSION_COOKIE may have expired — re-extract it from your browser session.`);
                }
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            if (isSourceUrl(url) && (error.response?.status === 401 || error.response?.status === 403)) {
                throw new Error(`removed.invalid returned ${error.response.status}. Set SESSION_COOKIE env var with your authenticated session cookie.`);
            }
