...
export function createMcpServer() {
    ...
    server.registerTool("search_cases", {
        ...
        if (jadeOutcome.status === "rejected")
    ...
    const jadeLookupShape = {
        by: z
            ...
            .describe("Lookup key: article_id resolves metadata for a numeric jade.io article ID; citation builds a jade.io lookup URL for a neutral citation"),
        articleId: z
            .number()
            .int()
            .min(1)
    ...
    server.registerTool("search_citing_cases", {
        ...
        description: "Find cases that cite a given case on jade.io. Uses jade.io's LeftoverRemoteService citator. Requires JADE_SESSION_COOKIE. Returns citing cases with neutral citations, case names, jade.io URLs, and the total count of citing cases. Results are a sample (typically 20-30) of the full set.",
        ...
        const lines = [
            ...
            "",
            ...results.map((r) => `- ${r.caseName} ${r.neutralCitation}${r.reportedCitation ? "; " + r.reportedCitation : ""} — ${r.jadeUrl}`),
        ];
    ...
    server.registerTool("cache_cited_by", {
        ...
        description: "Fetch citing cases for a cached citation from jade.io and store them locally. " +
                ...
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({
                            error: "jade.io returned no results but existing cited-by data is present. " +
