...
describe("mode/op/action/by dispatch validation", () => {
  it("jade_lookup rejects by=article_id without articleId", async () => {
    const client = await connectedClient();
  ...
  it("jade_lookup rejects by=citation without citation", async () => {
    const client = await connectedClient();
    const result = await client.callTool({
      name: "jade_lookup",
...
describe("offline dispatch paths", () => {
  ...
  it("jade_lookup by=citation builds a lookup URL without network access", async () => {
    const client = await connectedClient();
    const result = await client.callTool({
      name: "jade_lookup",
    ...
    expect(result.isError).toBeFalsy();
    const text = (result.content as Array<{ type: string; text: string }>)[0]!.text;
    const parsed = JSON.parse(text) as { citation: string; jadeUrl: string };
    expect(parsed.citation).toBe("[2008] NSWSC 323");
    expect(parsed.jadeUrl).toContain("jade.io");
  });
});
