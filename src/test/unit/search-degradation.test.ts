...
vi.mock("../../services/jade.js", () => ({
  buildCitationLookupUrl: vi.fn((citation: string) => `https://jade.io/search/${citation}`),
  resolveArticle: vi.fn(),
  searchCitingCases: vi.fn(),
  searchJadeWithStatus: mocks.searchJadeWithStatus,
}));
...
describe("AustLII search degradation", () => {
  ...
  it("search_cases returns jade results when AustLII search is blocked", async () => {
    const jadeResult: SearchResult = {
    ...
    mocks.searchJadeWithStatus.mockResolvedValueOnce({ results: [jadeResult], status: "ok" });
    ...
    try {
      ...
      expect(parsed.sources).toEqual({ austlii: "blocked", jade: "ok" });
      expect(parsed.results).toHaveLength(1);
      expect(parsed.results[0]!.source).toBe("jade");
  ...
  it("search_cases keeps array JSON when AustLII succeeds and jade is not configured", async () => {
    ...
    mocks.searchJadeWithStatus.mockResolvedValueOnce({
      results: [],
  ...
  it("search_cases reports when jade is not configured during an AustLII block", async () => {
    mocks.searchAustLii.mockRejectedValueOnce(
      new CloudflareBlockedError("https://www.austlii.edu.au/cgi-bin/sinosrch.cgi", false),
    );
    mocks.searchJadeWithStatus.mockResolvedValueOnce({
    ...
    try {
      ...
      expect(result.isError).not.toBe(true);
      expect(mocks.searchJadeWithStatus).toHaveBeenCalled();
      const parsed = JSON.parse(firstText(result)) as {
        results: SearchResult[];
        warnings: Array<{ code: string; source: string; message: string }>;
        sources: Record<string, string>;
  ...
  it("search_cases reports when jade fails during an AustLII block", async () => {
    mocks.searchAustLii.mockRejectedValueOnce(
      new CloudflareBlockedError("https://www.austlii.edu.au/cgi-bin/sinosrch.cgi", false),
    );
    ...
    try {
      ...
      expect(parsed).toMatchObject({
        results: [],
        degraded: true,
        sources: { austlii: "blocked", jade: "failed" },
      });
      expect(parsed.warnings[0]?.code).toBe("austlii_cloudflare_blocked");
    } finally {
      await Promise.allSettled([client.close(), server.close()]);
  ...
  it("search_cases does not silently swallow unexpected jade failures", async () => {
    mocks.searchAustLii.mockResolvedValueOnce([]);
    mocks.searchJadeWithStatus.mockRejectedValueOnce(new Error("jade transport failed"));
    ...
    try {
      ...
      expect(text).toContain("jade transport failed");
    } finally {
      await Promise.allSettled([client.close(), server.close()]);
    }
  });
